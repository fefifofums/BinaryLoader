...
static bdd* open_bfd(std::string &fname)
{
  ...
  if(!bfd_h) {
    fprintf(stderr, "failed to open binary '%s' (%s)\n", fname.c_str(),
        bfd_eermsg(bfd_get_error()));
    return NULL;
  }

  if(!bfd_check_format(bfd_h, bfd_object)) {
    fprintf(stderr, "file '%s' does not look like an executable (%s)\n",
        fname.c_str(), bfd_eermsg(bfd_get_error()));
    return NULL;
  }
...
static int load_sections_bfd(bfd *bfd_h, Binary *bin)
{
  ...
  for(bfd_sec = bfd_h->sections; bfd_sec; bfd_sec->next) {
